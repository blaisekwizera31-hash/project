...
const iconMap = [User, Briefcase, FileCheck, Scale, BarChart3];
const colorMap = [
    ...
    <section id="roles" className="py-24">
      <div className="container mx-auto px-4">
        <motion.div
          ...
          className="text-center mb-16"
        >
          <span className="text-accent font-semibold text-sm uppercase tracking-wider">{t.label}</span>
          <h2 className="text-3xl md:text-4xl font-display font-bold mt-2 mb-4">
            {t.title}
          </h2>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            ...
            return (
              ...
              >
                <div className="bg-card rounded-2xl p-8 h-full shadow-soft border border-border/50 hover:shadow-elevated transition-all duration-300 group">
                  ...
                  <Button variant="ghost" className="group/btn p-0 h-auto text-accent hover:text-accent">
                    {commonT.getStarted} <ArrowRight className="w-4 h-4 ml-1 group-hover/btn:translate-x-1 transition-transform" />
                  </Button>
                </div>
              </motion.div>
            );
